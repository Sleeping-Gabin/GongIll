package com.gabin.gongill.view.model
...
import com.gabin.gongill.CompetitionRepository
...
class MyViewModel(application: Application) : AndroidViewModel(application) {
	...
	private fun updateTeamPlayList() {
		...
		if (!playList.value.isNullOrEmpty() && !teamList.value.isNullOrEmpty()) {
			_teamPlayList.value = teamList.value?.associateWith { t ->
				playList.value?.filter { p -> p.group == t.groupName && (t.alias == p.team1 || t.alias == p.team2) }
					?: listOf()
			}
		}
	}
