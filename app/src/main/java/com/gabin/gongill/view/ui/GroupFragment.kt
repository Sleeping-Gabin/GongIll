...
import com.gabin.gongill.databinding.AddGroupDialogBinding
import com.gabin.gongill.databinding.AddTeamDialogBinding
import com.gabin.gongill.databinding.GroupFragmentBinding
import com.gabin.gongill.view.adapter.CategoryAdapter
import com.gabin.gongill.view.listener.OnGroupTouchListener
...
class GroupFragment : Fragment(), OnGroupTouchListener {
	...
	override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
		...
		binding.groupView.layoutManager =
	...
	override fun onTouchButton(group: Group) {
		...
		dialogBinding.addTeamDialogTeamNameText.addTextChangedListener(object : TextWatcher {
			...
			override fun beforeTextChanged(s: CharSequence?, start: Int, count: Int, after: Int) {
				...
				isSame =
					name?.subSequence(0, min(name.length, dialogBinding.addTeamDialogAlias.counterMaxLength))
						.toString() ==
		...
		dialogBinding.addTeamDialogAliasText.doOnTextChanged { text, _, _, _ ->
			if (text != null && text.length > dialogBinding.addTeamDialogAlias.counterMaxLength)
				dialogBinding.addTeamDialogAlias.error = getString(
					R.string.error_alias_maxCount, dialogBinding.addTeamDialogAlias.counterMaxLength
				)
			else
	...
	override fun onTouchItem(group: Group) {
		...
		Navigation.findNavController(requireActivity(), R.id.hostFragment)
			.navigate(R.id.action_groupFragment_to_groupItemFragment)
	}
